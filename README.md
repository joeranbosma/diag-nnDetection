...
The docker container expects data and models in its own `/output/data` and `/output/models` directories respectively.
...
```bash
docker run --gpus all -v ${det_data_parent}:/output -v ${det_models_parent}:/output -it --shm-size=24gb nndetection:0.1 /bin/bash
```
